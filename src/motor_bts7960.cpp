...
uint32_t lastDebugTime_motor = 0;
...
void motorSetup()
{
    Serial.println("Motor Setup Start");
    // ===== INPUT =====
...
void motorLoop()
{
    ...
    // ===== logic =====
    if (ch1 > 1500)
        throttle = map(ch1, 1500, 2000, 0, 255);
    else if (ch1 < 1500)
        throttle = map(ch1, 1000, 1500, -255, 0);

    if (ch2 > 1500)
        steering = map(ch2, 1500, 2000, 0, 255);
    else if (ch2 < 1500)
        steering = map(ch2, 1000, 1500, -255, 0);
    ...
    // ===== DEBUG =====
    if (millis() - lastDebugTime_motor > 100)
    {
        lastDebugTime_motor = millis();

        Serial.print("[MOTOR] CH1: ");
        Serial.print(ch1);
        Serial.print(" CH2: ");
        Serial.print(ch2);
        ...
        Serial.print(" | Signal: ");
        Serial.print(signalValid ? "OK" : "LOST");

        Serial.print(" | Failsafe: ");
        Serial.println((millis() - lastSignalTime_motor > 300) ? "ON" : "OFF");
    }
